...
public:
    ...
    template<typename T>
    ArgHolder(std::string str, const T& value) 
        : index(str.back())
        , value(std::make_unique<Derived<T>>(value)) {}
    
    ArgHolder(const ArgHolder& other)
        : index(other.index)
        , value(other.value->get_copy()) {}
    
    ArgHolder(ArgHolder&& other)
        : index(other.index)
        , value(std::move(other.value)) {}


    ArgHolder& operator=(const ArgHolder& other) {
        if(this == &other) {
            return *this;
        }
    ...
    ArgHolder& operator=(ArgHolder&& other) {
        if(this == &other) {
            return *this;
        }
        value = std::move(other.value);
